...
// Each of these arrays is still fully seeded with CamCCUL's own
// "[... — to be provided]" placeholder copy — hide the whole section
// rather than show fabricated history, leadership, or mission/vision text.
const hasRealMilestones = milestones.some(
  (m) => !isPlaceholder(m.title.en) && !isPlaceholder(m.description.en)
);
const hasRealMission = !isPlaceholder(mission.en);
const hasRealVision = !isPlaceholder(vision.en);
const hasRealLeadership = leadership.some(
  (member) => !isPlaceholder(member.name.en) && !isPlaceholder(member.bio.en)
);
...
export function AboutPageClient() {
  ...
  return (
    <>
      ...
      {/* SECTION 1: OUR STORY — hidden until CamCCUL provides real milestone copy */}
      {hasRealMilestones && (
          ...
          <div className="max-w-5xl mx-auto px-4">
            ...
            <div className="relative mt-16">
              ...
              <div className="space-y-12">
                {milestones
                  .filter((m) => !isPlaceholder(m.title.en) && !isPlaceholder(m.description.en))
                  .map((milestone, index) => {
      ...
      {/* SECTION 2: MISSION & VISION — hidden until CamCCUL provides real copy */}
      {(hasRealMission || hasRealVision) && (
          ...
          <div className="max-w-7xl mx-auto px-4">
            ...
            <div
              className={cn(
                ...
                hasRealMission && hasRealVision ? "md:grid-cols-2" : "max-w-2xl mx-auto"
              )}
            >
              {hasRealMission && (
      ...
      {/* SECTION 3: LEADERSHIP — hidden until CamCCUL provides real board bios */}
      {hasRealLeadership && (
      <section className="bg-white py-20">
        <div className="max-w-7xl mx-auto px-4">
          <SectionHeader
          ...
          <div className="mt-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {leadership
              .filter((member) => !isPlaceholder(member.name.en) && !isPlaceholder(member.bio.en))
              .map((member, index) => (
                <div
      ...
      <section className="bg-gray-50 py-20">
        <div className="max-w-7xl mx-auto px-4">
          <SectionHeader
            ...
            subtitle={t("about_presence_subtitle").replace("{count}", String(affiliateCount))}
          />
